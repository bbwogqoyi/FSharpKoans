    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        ...
        let length (xs : 'a list) : int =
          let rec innerLength xs count = 
            match xs with
            | _ :: tail -> innerLength tail (count+1)
            | [] -> count 
    ...
    let ``03 Reversing a list, the hard way`` () =
        ...
        let rev (xs : 'a list) : 'a list =
          let rec innerRev initList revList = 
            match initList with 
            | head :: tail -> innerRev tail (head :: revList)
            | [] -> revList
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        ...
        let map (xs : int list) : int list =
          let rec innerMap initList mapList = 
            match initList with 
            | head :: tail -> 
              let value = head + 1
              innerMap tail (mapList @ [value])
            | [] -> mapList
    ...
    let ``09 Specified-function filtering, the hard way`` () =
      // write a function which filters based on the specified criteria
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
          let rec innerFilter initList mapList = 
            match initList with 
            | head :: tail -> 
              match (f head) with
              | true -> innerFilter tail (mapList @ [head])
              | _ -> innerFilter tail mapList
            | [] -> mapList
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
          let rec innerFold currentState _list =
            match _list with 
            | head :: tail -> innerFold (currentState + head) tail
            | [] -> currentState

          innerFold initialState xs
    ...
    let ``16 Folding, the hard way`` () =
      // write a function to do a fold.
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
          let rec innerFold currentState _list =
            match _list with
            | head :: tail -> innerFold (f currentState head) tail
            | [] -> currentState
    ...
    let ``18 exists: finding whether any matching item exists`` () =
      // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
          let rec innerExists _list =
            match _list with
            | [] -> false
            | head :: tail ->
              match (f head) with 
              | false -> innerExists tail
              | _ -> true
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        ...
        let init (n : int) (f : int -> 'a) : 'a list =
          let rec initHelper index _list =
            match index < n with
            | false -> _list 
            | _ -> initHelper (index+1) (_list @ [f index])
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
      // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
          let rec tryFindHelper _list =
            match _list with
            | [] -> None
            | head :: tail ->
              match (p head) with
              | true -> Some head
              | _ -> tryFindHelper tail
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
          ...
          let rec chooseHelper _in _out =
            match _in with 
            | [] -> _out
            | head :: tail -> 
              match (p head) with 
              | None -> chooseHelper tail _out
              | Some value -> chooseHelper tail (_out @ [value])
          chooseHelper xs []
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        ...
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
          let rec mapIndexOp index _in _out =
            match _in with
            | [] -> _out
            | head :: tail -> mapIndexOp (index+1) tail (_out @ [f index head])
          mapIndexOp 0 xs []
