...
module ``04: Match expressions`` = 
    ...
    let ``01 Basic match expression`` () =
        match 8000 with
        | _ -> "Insufficient power-level"  //either a wildcard or constant or identifier pattern can work
        ()
    ...
    let ``02 Match expressions are expressions, not statements`` () =
        let result =
            match 9001 with
            | num -> // <-- use an identifier pattern here!
                match num + 1000 with // <-- now use the identifier that you've bound
